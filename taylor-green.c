...
int main() {
  ...
  /**
  Space and time are dimensionless. The domain is a unit 2 pi square, centered on
...
event logfile (i++) {
  ...
  foreach() {
    ...
    ke[] = (0.5*sq(u.x[]) + 0.5*sq(u.y[]))/(4*pi*pi);

    double ft_2 = exp(-4*0.001*t);

    double u0 = sin(x)*cos(y)*ft_2;
    double v0 = - cos(x)*sin(y)*ft_2;

    e[] = sq(norm(u) - sqrt(sq(u0) + sq(v0)));
  }
  norm n = normf (e);
  printf ("%d %d %g %g %g\n", N, i, t, sqrt(n.avg), statsf(ke).sum);
}
...
event error (t = 20) {
  ...
  scalar e[];
  foreach() {
    double u0 = sin(x)*cos(y);
    double v0 = - cos(x)*sin(y);
